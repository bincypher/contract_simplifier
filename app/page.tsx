...
export default function Home() {
  ...
  const choose = (candidate?: File) => { setError(""); setResult(null); if (!candidate) return; if (candidate.type !== "application/pdf") return setError("Please choose a PDF document."); if (candidate.size > 15 * 1024 * 1024) return setError("This file exceeds the 15 MB limit."); setFile(candidate); };
  ...
  return <main>
    ...
    <section className="upload-wrap"><div className={`dropzone ${file ? "selected" : ""}`} onDragOver={e => e.preventDefault()} onDrop={onDrop} onClick={() => input.current?.click()}><input ref={input} type="file" accept="application/pdf" onChange={(e: ChangeEvent<HTMLInputElement>) => choose(e.target.files?.[0])} /><div className="file-icon">⌁</div><div><h2>{file ? file.name : "Drop your PDF here"}</h2><p>{file ? `${(file.size / 1024 / 1024).toFixed(2)} MB · Ready for analysis` : "or click to browse · PDF only · maximum 15 MB"}</p></div>{file && <button className="clear" onClick={e => { e.stopPropagation(); setFile(null); setResult(null); }}>×</button>}</div>
